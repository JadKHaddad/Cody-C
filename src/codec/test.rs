...
mod tokio_compat {
    ...
    use crate::{
        decode::decoder::Decoder, encode::encoder::Encoder, test::init_tracing, tokio::CodecCompat,
    };

    use futures::{sink::SinkExt, StreamExt};

    use super::FrameSizeAwareCodec;

    async fn sink_stream<E, D, I>(encoder: E, decoder: D, items: Vec<I>)
    where
        E: Encoder<I> + Send + 'static,
        <E as Encoder<I>>::Error: core::convert::From<tokio::io::Error> + Debug,
        D: Decoder<Item = I>,
        <D as Decoder>::Error: core::convert::From<tokio::io::Error>,
        I: Clone + Debug + PartialEq + Send + 'static,
    {
        let encoder_compat = CodecCompat::new(encoder);
        let decoder_compat = CodecCompat::new(decoder);

        let (read, write) = tokio::io::duplex(1024);

        let items_clone = items.clone();
        let handle = tokio::spawn(async move {
            let mut framed_write = FramedWrite::new(write, encoder_compat);
        ...
        let framed_read = FramedRead::new(read, decoder_compat);
        let collected_items: Vec<_> = framed_read
            .collect::<Vec<_>>()
            .await
            .into_iter()
        ...
        handle.await.unwrap();
        assert_eq!(items, collected_items);
    }

    #[tokio::test]
    async fn frame_size_aware() {
        init_tracing();
        ...
        sink_stream(FrameSizeAwareCodec, FrameSizeAwareCodec, items).await;
    }
}
